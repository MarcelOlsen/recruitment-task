...
import { starApiPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [starApiPlugin],
    });

    await request(server).get('/api/star-api/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/star-api/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/star-api/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/star-api/todos/${createdTodoItem.id}`)
